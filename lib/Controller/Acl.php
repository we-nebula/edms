...
class Controller_ACL extends \AbstractController {
	...
	function init(){
		...
		if(($crud = $this->getView()) instanceof \CRUD){
			...
			if($view instanceof \Grid){
				...
				$view->addMethod('format_action',function($g,$f){
					...
					if(!isset($g->current_row_html['action']))
						$g->current_row_html['action'] = $this->add('View_ActionBtn',['actions'=>$action_btn_list,'id'=>$g->model->id,'status'=>$g->model['status']?:"All",'action_btn_group'=>$this->action_btn_group,'status_color'=>$this->status_color])->getHTML();
	...
	function canAdd(){
		return (
					false //added to bypass further conditions
					&&(
					($this->app->ACLModel === 'Departmental' &&  $this->app->immediateAppove($this->model_ns))
					||
					$this->app->ACLModel ==='none'
	...
	function canDo(){
		...
		$this->acl_m = $this->add('Model_ACL',['for_model'=>$this->model])
	...
	function textToCode($text){
		// if config to department
			...
			// if $ns is allowed to me return [$this->app->employee->id];
				// else
						// return false;
		if(false && $this->app->ACLModel ==='Departmental' &&  !$this->app->immediateAppove($this->model_ns)){	
			return false;	
		} 
	...
	function manageSpotACLVP($vp){
		$vp->set(function($page){
			...
			if(false && $this->app->ACLModel === "Departmental"){
			...
			if($dt){
				...
				try{
					...
					try{
						...
						try{
							...
							$m = $this->add('Model_ConfigJsonModel',['fields'=>[1],'config_key'=>$dt]);
							$is_config = true;
						}
					}
				...
				$existing_acl = $this->add('Model_ACL')
				...
				foreach ($m->actions as $status => $actions) {
					...
					foreach ($actions as $action) {
						...
						$tf = $greeting_card->addField('RadioButton',$status.'_'.$action,$action)
